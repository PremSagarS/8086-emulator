...
def _evaluate_labels(exec: Executable) -> None:
    ...
    for segment, data in exec.segment_space.items():
        ...
        for index in range(segment_length):
            ...
            if (
                current_ins[0] in instructions.control_transfer
                and current_ins[-1] in exec.labels
            ):
                ...
                if current_ins[1] == "FAR":
                    ...
                    label_adr = (
                        exec.labels[current_ins[1]]["seg"]
                        + ":"
                        + exec.labels[current_ins[1]]["offset"]
                    )
                    exec.segment_space[segment][index][1] = label_adr
                else:
                    exec.segment_space[segment][index][1] = exec.labels[current_ins[1]][
                        "offset"
                    ]
                ...
                for s in ["SEG", "OFFSET"]:
                    if word == s:
                        ...
                        if current_ins[i] in exec.labels.keys():
                            exec.segment_space[segment][index][i] = exec.labels[
                                current_ins[i]
                            ][s.lower()]
                        else:
                            exec.segment_space[segment][index][i] = exec.variables[
                                current_ins[i]
                            ][s.lower()]
...
def _assembleSegment(ip: int, exec: Executable) -> int:
    ...
    exec.segment_space[segment] = [["0"]] * int(
        "10000", 16
    )  # fill segment with empty lists instead of just zeroes
    ...
    for segment_ip in range(ip + 1, len(exec.statements)):
        ...
        if currentStatement[0] == segment_label:
            ...
            exec.segment_space[segment][
                segment_ip : segment_ip + len(varBytes)
            ] = varBytes
            relative_ip += len(varBytes)

        elif (
            len(currentStatement) > 2
            and currentStatement[1] in directives.data_definition
        ):
            ...
            withoutName = currentStatement_raw.replace(
                currentStatement_raw.split()[0], "", 1
            ).strip()
            varBytes = _bytes(currentStatement[1:], withoutName)
            exec.segment_space[segment][
                relative_ip : relative_ip + len(varBytes)
            ] = varBytes
            relative_ip += len(varBytes)
...
def _bytes(dir: list[str], dir_raw: str) -> list[str]:
    ...
    if len(dir) > 2 and dir[2][:3] == "DUP":
        ...
        repeat = [
            s for s in re.split(r"[ |,]", repeat_raw.strip().upper()) if s
        ]  # ['DB', 'VAL1', '\'VAL2\'', '30h']
        return _bytes(repeat, repeat_raw) * repeatVal

    elif varType == "DB":
        vals_raw = utils.replaceNIQ(
            dir_raw, varType_raw, ""
        ).strip()  # val1, 'val2', 30h
...
def _getSegmentAddressability(statements: list[list[str]]) -> dict[str, str]:
    ...
    for statement in statements:
        if statement[0] == "ASSUME":
            ...
            break
    if not found:
        addressability = {"CODE": "CS", "DATA": "DS"}
...
def _prep(asm: str) -> tuple[list[list[str]], list[str]]:
    ...
    for line in asm.split(os.linesep):
        ...
        line = re.sub(r'(\'[^\']*\'|"[^"]*")|(?P<nq>\w)', _upper, line.strip())
