...
class DataSegModel(DataModel):
    def __init__(self, BIU: BIU, parent=None):
        super(DataSegModel, self).__init__(
            ["Address", "Physical Address", "Value"], parent
        )

        for phy in range(256):
            adr = BIU.registers["DS"] * 16 + phy
            item = [hex(phy), hex(adr)]
            info = BIU.read_byte(adr)
