...
class EU:
    def __init__(self, bus: BIU, debug: bool) -> None:
        self.bus = bus
        self.IR: list[str] = []
    ...
    def write_register(self, register: str, value: int) -> None:
        value = (
            utils.unsigned(value, self.operand_size) & 0xFFFF
        )  # anding so num fits in 2 bytes
        if register in self.gpr:
            ...
            if register[1] == "L":
                ...
                full_reg = register[0] + "X"
                self.gpr[full_reg] = (self.gpr[full_reg] & 0x00FF) + (
                    (value << 8) & 0xFF00
                )
    ...
    def increment_register(self, register: str, value: int) -> None:
        self.write_register(register, self.read_register(register) + value)

    def fetch_address(self, operand: str) -> int:
        out = 0
    ...
    def write_memory(self, address: int, data: str) -> None:
        if self.operand_size == 1:
    ...
    def fetch_operand(self, operand: str | int) -> int:
        if isinstance(
            operand, int
        ):  # when called from within eu, and not from an asm string
            operand = "[" + str(operand) + "]"
    ...
    def write_operand(self, operand: str, data: int) -> None:
    ...
    def is_register(self, string: str) -> bool:
        return string in (self.gpr_all + self.bus_reg)

    def is_address(self, string: str) -> bool:
        return "[" in string

    def interrupt_handler(self, type: int):
        ...
        if self.debug_interrupts:
            ...
            self.print(
                f"Read interrupt vector table {hex(type * 4)} offset address {hex(ip_val)} => IP\n"
            )
            self.print(
                f"Read interrupt vector table {hex(type * 4 + 2)} Branch address {hex(cs_val)} => CS\n"
            )
