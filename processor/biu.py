...
class BIU:
    ...
    def __init__(self, queue_size: int, memory: Memory) -> None:
        ...
        self.registers = {
            ...
            "IP": int(str(memory.executable.ip), 16),
        }

        self.prefetch_ip = int(str(self.memory.executable.ip), 16)
    ...
    def read_byte(self, address: int) -> list[str]:
        return self.memory.read_byte(address)

    def read_word(self, address: int) -> list[list[str]]:
        return self.memory.read_byte(address + 1) + self.memory.read_byte(address)

    def read_dword(self, address: int) -> list[list[str]]:
        return (
            self.memory.read_byte(address + 3)
    ...
    def write_dword(self, address: int, data: int) -> None:
        if isinstance(data, int):
            self.write_byte(address, data & 0xFF)
            self.write_byte(address + 1, (data >> 8) & 0xFF)
